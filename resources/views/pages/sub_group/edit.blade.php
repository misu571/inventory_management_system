...
@section('content')
<x-pages.elements.title title="Form" route="{{ route('sub-group.index') }}" parentPage="sub category" currentPage="edit" />
<div class="row">
    <div class="col-md-4">
        <div class="card-box p-3 mb-30">
            <form id="sub_group-form" method="POST" action="{{ route('sub-group.update', [$subGroup->id]) }}">
                @method('PATCH')
                @csrf
                <div class="row">
                    <div class="col">
                        <x-forms.type.select-single-input id="category" label="Category" name="category" validations="required">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected((old('category') ?? $subGroup->category_id)==$category->id)>{{ $category->name }}</option>
                            @endforeach
                        ...
                        <x-forms.type.text-input type="text" id="name" label="Name" name="name" classes="" value="{{ old('name') ?? $subGroup->name }}" validations="required" />
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        if ("{{ old('category') }}") {
            ...
            getSubCategories({{ $subGroup->category_id }}, {{ $subGroup->sub_category_id }})
        }
    })
