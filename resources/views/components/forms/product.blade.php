<form method="POST" {{ $attributes }}>
    ...
    <div class="row">
        <div class="col">
            ...
            <x-forms.type.select-single-input id="category_id" label="Category" name="category" validations="required">
                <x-slot:select>@if(!isset($method_type)) selected @endif</x-slot>
                {{ $categories }}
            </x-forms.type.select-single-input>
            <x-forms.type.select-single-input id="supplier_id" label="Supplier" name="supplier" validations="required">
                <x-slot:select>@if(!isset($method_type)) selected @endif</x-slot>
                {{ $suppliers }}
            </x-forms.type.select-single-input>
        </div>
